-- 0057_debug_temp_trace_submit.sql
-- 一時デバッグ用（0056の続き）。同一クエリを単体で実行すると成功する
-- （0056で確認済み）のに、submit_current_version経由では実行後も
-- duplicate_flagsが作られない。submit_current_version自体の実行過程で
-- 実際にv_profileがどう評価されているかをテーブルに記録し、直接見る。
...
begin
  ...
  -- ★デバッグ：この時点のv_participation/v_profileの実際の値を記録する
  insert into debug_trace (label, payload)
  values ('pre-duplicate-insert', jsonb_build_object(
    'v_participation_id', v_participation.id,
    'v_participation_exhibitor_profile_id', v_participation.exhibitor_profile_id,
    'v_profile_id', v_profile.id,
    'v_profile_company_name', v_profile.company_name,
    'v_profile_normalized', normalize_company_name_for_matching(v_profile.company_name)
  ));
  ...
  -- ★デバッグ：company_name_similarity挿入直後の件数を記録する
  insert into debug_trace (label, payload)
  values ('post-duplicate-insert', jsonb_build_object(
    'flag_count_now', (select count(*) from duplicate_flags where event_id = v_participation.event_id)
  ));
