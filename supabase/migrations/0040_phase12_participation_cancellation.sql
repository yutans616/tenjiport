-- 0040_phase12_participation_cancellation.sql
-- 出展者のキャンセル機能：status='cancelled'・cancelled_at・cancelled_reasonは
-- スキーマ上存在していたが、これまで書き込む経路がどこにも無かった。
-- 併せて、キャンセル済み・統合済みの参加からの再提出を submit_current_version で拒否する
-- （防がないと、出展者側の開いたままのタブから再提出されると is_billable=true に巻き戻り、
-- キャンセルが実質的に無効化されてしまうため）。

create or replace function cancel_event_participation(p_event_participation_id uuid, p_reason text)
returns event_participations
language plpgsql
security definer
...
begin
  ...
  select * into v_participation from event_participations where id = p_event_participation_id;
  if v_participation.status in ('cancelled', 'merged') then
    raise exception 'already cancelled or merged';
  end if;

  update event_participations
  set status = 'cancelled', cancelled_at = now(), cancelled_reason = p_reason, updated_at = now()
  where id = p_event_participation_id
  ...
  values (auth.uid(), v_org_id, 'cancel_event_participation', 'event_participation', v_participation.id, to_jsonb(v_participation));

  return v_participation;
end;
$$;

-- submit_current_version（0035版に、参加ステータスの再提出ガードを1ブロック追加したもの）。
