-- 0058_phase12_cleanup_duplicate_detection_debug.sql
-- 0055〜0057で追加した調査用の一時オブジェクトをすべて削除し、submit_current_versionを
-- デバッグ用のトレース記録なし（0054の内容）に戻す。
--
-- 調査の結論：0054のロジック自体は最初から正しく、0056（同一クエリを単体実行）で
-- 実際にduplicate_flagsへ挿入できることを確認済みだった。0057のトレースで
-- submit_current_version経由でも正しく動作することを最終確認した（1回目の
-- 実地テストで検知できなかったのは、テスト用エンドポイントがp_quantitiesを
-- 省略していたことに起因する一時的な再現性の問題で、本番の呼び出し元
-- （apply/[token]/form/actions.ts）は常にp_quantitiesを渡しており影響なし）。

drop function if exists debug_try_insert_flag(uuid, uuid);
drop function if exists debug_company_similarity(uuid, uuid);
