-- 0030_phase10_bulk_invoices.sql
-- Phase C: 請求書の一括発行。既存のcreate_exhibitor_invoice（0010）と同じロジックを
-- 対象者ごとに繰り返す。resolved_price_yen（Phase Bでフォームの価格付き選択肢から
-- 自動計算される金額）が未設定、またはすでに請求書が存在する参加者はスキップする。
-- オーナー・管理者限定（呼び出し元のアプリ側でも同じ判定をしているが、RPC側でも
-- 二重にチェックする）。
...
begin
  ...
  foreach v_pid in array p_event_participation_ids loop
    ...
    select resolved_price_yen into v_price from event_participations where id = v_pid;
    select id into v_existing_invoice_id from exhibitor_invoices where event_participation_id = v_pid order by created_at desc limit 1;
